...
function renderEditForm({src,setSrc,score,onIngChange,onImgUpload,uploading,imgRef,onSave,onCancel,onDelete,saving,saveLabel,cancelLabel,CATEGORIES,SKIN_TYPES,scoreColor,toggleSkin,prefilling=false}) {
  if (!src) return null;
  // Format enrichment timestamp as relative time ("3 days ago" / "just now")
  const enrichedRelTime = src.lastEnrichedAt ? (() => {
    const ms = Date.now() - src.lastEnrichedAt;
    ...
    if (wks < 4) return `${wks} wk${wks===1?"":"s"} ago`;
    return new Date(src.lastEnrichedAt).toLocaleDateString();
  })() : null;
  return (
...
function AdminNuclearClean({ onBack }) {
  ...
  async function runNuclearClean() {
    ...
    // Use existing Firestore products as the seed — just clean and normalize them
    const seedData = surviving;
    addLog(`Normalizing ${seedData.length} surviving products`,"info");
    for (const seed of seedData) {
      if (stopRef.current) break;
      // Normalize this existing product — fix casing, remove trailing spaces
      const cleanName = (seed.productName||"").trim();
      const cleanBrand = (seed.brand||"").trim();
      if (!cleanName) continue;
      const updates = { productName: cleanName, brand: cleanBrand, updatedAt: Date.now() };
      // Fix common category issues
      const catMap = {"serum":"Serum","moisturizer":"Moisturiser","moisturiser":"Moisturiser","face-wash":"Face Wash","face wash":"Face Wash","spf":"SPF","toner":"Toner","eye cream":"Eye Cream","eye":"Eye Cream","mask":"Mask","acne":"Acne Treatment","acne treatment":"Acne Treatment","body":"Body","hair":"Hair","lip":"Lip","exfoliant":"Toner","other":""};
      const rawCat = (seed.category||"").trim();
      updates.category = catMap[rawCat.toLowerCase()] || rawCat;
      // Normalize skinTypes array
      if (Array.isArray(seed.skinTypes) && seed.skinTypes.length) {
        updates.skinTypes = seed.skinTypes;
      } else if (typeof seed.skinTypes === "string" && seed.skinTypes.trim()) {
        updates.skinTypes = seed.skinTypes.split(",").map(s=>s.trim().replace(/^"+|"+$/g,"")).filter(Boolean);
      }
      try {
        await updateDoc(doc(db,"products",seed.id),updates);
        addLog(`✓ Normalized: "${cleanName}"`,"ok"); s.cleaned++;
      } catch(e) { addLog(`✗ Failed: "${cleanName}": ${e.message}`,"error"); }
...
function FixAmazonUrls({ products, onFixed }) {
  // Stub — Amazon URL fixing removed in cleanup simplification
  return null;
}
...
function AdminImagePicker({ products, setProducts, onBack }) {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:"0.75rem"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:T.accent,fontSize:"0.72rem",cursor:"pointer",fontFamily:"'Inter',sans-serif",textAlign:"left",padding:0}}>← Back to Cleanup</button>
      <div style={{background:T.surface,borderRadius:"1rem",padding:"1.25rem",border:`1px solid ${T.border}`,textAlign:"center"}}>
        <div style={{fontSize:"0.9rem",fontWeight:"700",color:T.text,fontFamily:"'Inter',sans-serif",marginBottom:"0.5rem"}}>🔍 AI Image Picker</div>
        <div style={{fontSize:"0.75rem",color:T.textLight,fontFamily:"'Inter',sans-serif",marginBottom:"1rem"}}>Use the ✨ AI Enrich pipeline instead — it finds and uploads images automatically for any product.</div>
        <button onClick={onBack} style={{padding:"0.6rem 1.5rem",background:T.accent,color:"#fff",border:"none",borderRadius:"0.75rem",fontSize:"0.75rem",fontWeight:"600",cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>← Go to AI Enrich</button>
      </div>
    </div>
  );
}
